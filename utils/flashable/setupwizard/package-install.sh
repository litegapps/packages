...
MAGISK | magisk | magisk_module)
PROP_DIR=`echo "$(dirname $SYSTEM_INSTALL)/system.prop"`
touch $PROP_DIR
...
*)
#backup build.prop
if [ $SYSTEM_DIR/product/build.prop ]; then
	print "- Backuping build.prop"
	test ! -d $TMP/backup && cdir $TMP/backup
	cp -pf $SYSTEM_DIR/product/build.prop $TMP/backup/build.prop
fi
set_prop "setupwizard.feature.baseline_setupwizard_enabled" "true" "$SYSTEM_DIR/product/build.prop"
set_prop "ro.setupwizard.enterprise_mode" "1" "$SYSTEM_DIR/product/build.prop"
set_prop "ro.setupwizard.rotation_locked" "true" "$SYSTEM_DIR/product/build.prop"
set_prop "setupwizard.enable_assist_gesture_training" "true" "$SYSTEM_DIR/product/build.prop"
set_prop "setupwizard.theme" "glif_v3_light" "$SYSTEM_DIR/product/build.prop"
set_prop "setupwizard.feature.skip_button_use_mobile_data.carrier1839" "true" "$SYSTEM_DIR/product/build.prop"
set_prop "setupwizard.feature.show_pai_screen_in_main_flow.carrier1839" "false" "$SYSTEM_DIR/product/build.prop"
set_prop "setupwizard.feature.show_pixel_tos" "false" "$SYSTEM_DIR/product/build.prop"
set_prop "ro.setupwizard.network_required" "false" "$SYSTEM_DIR/product/build.prop"
;;
esac
